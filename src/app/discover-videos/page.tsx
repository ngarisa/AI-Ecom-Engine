...
export default function DiscoverVideosPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["discover-videos", query, niche, minDays, order, searchTrigger],
    queryFn: async () => {
      ...
      params.set("display_format", "video");
      params.set("limit", "40");

      const res = await fetch(`/api/foreplay/discover-ads?${params}`);
      if (!res.ok) throw new Error("Failed to discover video ads");
      return res.json();
    },
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2">
          ...
          Discover Winning Video Ads
        </h1>
        <p className="text-sm text-muted-foreground mt-1">
          Search across millions of video ads to find proven winners in any niche.
        </p>
      </div>
      ...
      {searchTrigger === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-lg font-medium">Search Video Ads</h3>
          <p className="text-sm text-muted-foreground mt-1 max-w-md">
            Enter a keyword or select a niche to discover winning video ads across millions of brands.
          </p>
        </div>
      ) : isLoading ? (
        ...
        <div className="text-center py-20 text-muted-foreground">
          No video ads found. Try different search terms or filters.
        </div>
      ) : (
